...
class Grid:
    ...
    def move(self, cmd):
        ...
        if direction == 'R':  # Need to move amount columns right from [0]
            for i in range(int(amount)):
                if not self.col[0] - 1 > self.columns:  # need to add column right
                    # Need to insert a column for all self.rows

                    rowsToAdd = [[amount] for j in range(self.rows)]
                    self.grid.insert(0, [rowsToAdd])
                    self.columns += 1
                else:
                    self.col[0] += 1

        if direction == 'U':  # Need to have amount rows up from [0]
            for i in range(int(amount)):
                if not self.row[0] - 1 > 0:  # need to add row above
                    colsToAdd = [[amount] for j in range(self.columns)]
                    self.grid.insert(0, colsToAdd)
                    self.rows += 1
                else:
                    self.row[0] += 1
